...
export interface Asset {
  ...
  asset_type: 'display_asset' | 'input_asset';
  asset_content_type: 'video' | 'threadbuilder' | 'article';
}
...
export const getFirstTask = (): Task | undefined => {
  return projectData.tasks[0];
};

export const getAssetsByType = (assetType: Asset['asset_type']): Asset[] => {
  const task = getFirstTask();
  if (!task) return [];
  return task.assets.filter(asset => asset.asset_type === assetType);
};

export const getAssetByContentType = (contentType: Asset['asset_content_type']): Asset | undefined => {
  const task = getFirstTask();
  if (!task) return undefined;
  return task.assets.find(asset => asset.asset_content_type === contentType);
};

export const getAssetByTitle = (title: string): Asset | undefined => {
  const task = getFirstTask();
  if (!task) return undefined;
  return task.assets.find(asset => asset.asset_title.toLowerCase().includes(title.toLowerCase()));
};
